                      ...
                      <div key={p.id} id={`prod-${p.id}`} className={`border-t border-line px-4 py-3 transition-colors lg:odd:border-r ${addId === p.id ? "bg-brand-50/40" : !p.active ? "bg-soft/40 hover:bg-soft/60" : "hover:bg-soft/25"}`}>
                        ...
                        <div className="mt-2 flex flex-col gap-1 border-l-2 border-line/70 pl-3">
                          ...
                          <div className="flex flex-wrap items-center gap-1.5 self-start pt-0.5">
                            {barcodes.filter((s) => !isDisc(p.name, s.size)).length > 0 && (
                              <select value="" onChange={(e) => { if (e.target.value) toggleDisc(p.name, e.target.value, true); }} className="h-7 rounded-md border border-line bg-white px-1.5 text-xs text-muted" title="ทำเครื่องหมายเลิกผลิต (เฉพาะขนาดที่ใช้อยู่)">
                                <option value="">⊘ เลิกผลิต…</option>
                                {barcodes.filter((s) => !isDisc(p.name, s.size)).map((s) => <option key={s.id} value={s.size}>{s.size.replace(/\.$/, "")}</option>)}
                              </select>
                            )}
                            {addId === p.id ? (
                              ...
                              <button onClick={() => openAdd(p.id)} className="inline-flex items-center gap-1 rounded-md border border-dashed border-brand/40 px-2 py-1 text-xs text-brand-600 hover:bg-brand-50"><Plus size={12} /> เพิ่มรายการ</button>
                            )}
                          </div>
                        </div>
