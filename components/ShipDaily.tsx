...
export default function ShipDaily({ rows, date }: { rows: ShipRow[]; date: string }) {
  ...
  function exportCsv() {
    const header = ["เวลา", "Order No.", "เลขที่ใบเบิก", "ผู้รับ", "จังหวัด", "รายการ", "ผู้สแกน"];
    const lines = rows.map((r) => [timeOf(r.shipped_at), r.order_no, r.doc_no || "", r.receiver || "", r.province || "", r.item_count, r.shipped_by_name || ""]
      ...
      <div className="overflow-hidden rounded-xl border border-line bg-white">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {rows.length === 0 && <tr><td colSpan={7} className="px-4 py-12 text-center text-muted"><Truck size={20} className="mx-auto mb-2 text-faint" />ไม่มีรายการส่งในวันนี้</td></tr>}
              {rows.map((r) => (
