            ...
            {items.map((it, i) => (
              <tr key={i} className={`border-t border-line align-top ${it.is_free ? "bg-brand-50/50" : ""}`}>
                ...
                <td className="px-3 py-2">
                  ...
                  {isTester(it) && testerProducts.length === 0 && <div className="mt-1 flex items-center gap-1 text-[11px] text-red-600"><AlertTriangle size={12} /> ยังไม่มีสต๊อก Try Me — <a href="/stock" target="_blank" rel="noreferrer" className="font-medium underline hover:text-red-700">รับสินค้าเข้าสต๊อก</a> ก่อน</div>}
      ...
      <div className="space-y-3 md:hidden">
        {items.map((it, i) => (
          <div key={i} className="card space-y-2 p-3">
            ...
            <Combobox value={it.product} onChange={(v) => setProduct(i, v)} options={productOptionsFor(it)} allowCustom={!sizeAllow && !isTester(it)} placeholder={isTester(it) ? "เลือกกลิ่น Try Me" : "เลือกกลิ่น"} invalid={errors[i]?.product} codes={productCodes} />
            {isTester(it) && testerProducts.length === 0 && <div className="flex items-center gap-1 text-xs text-red-600"><AlertTriangle size={12} /> ยังไม่มีสต๊อก Try Me — <a href="/stock" target="_blank" rel="noreferrer" className="font-medium underline hover:text-red-700">รับสินค้าเข้าสต๊อก</a> ก่อน</div>}
