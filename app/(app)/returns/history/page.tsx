...
import { listReturns, returnStatsByScent } from "@/lib/queries";
...
export default async function ReturnsHistoryPage() {
  ...
  const [rows, stats] = await Promise.all([listReturns(), returnStatsByScent()]);
  return (
    <div className="mx-auto max-w-6xl px-4 py-6 md:px-8">
      ...
      <ReturnsHistory rows={rows} stats={stats} canReverse={can.manageDamaged(me.role)} />
    </div>
  );
}
