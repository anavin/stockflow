...
import { getOrder, getProducts, getSizes, getProvinces, getPostcodes, getProductCodes, getProductTypes, getBlockedSizesForOrder, getDiscInStock, getTesterStock, getOrderFormCatalog, listPacks } from "@/lib/queries";
...
export default async function EditOrderPage({ params }: { params: Promise<{ platform: string; orderNo: string }> }) {
  ...
  const [order, products, sizes, provinces, postcodes, productCodes, productTypes, discontinued, discInStock, testerStock, wsCat, packs] = await Promise.all([
    getOrder(decoded), getProducts(), getSizes(), getProvinces(), getPostcodes(), getProductCodes(), getProductTypes(), getBlockedSizesForOrder(), getDiscInStock(), getTesterStock(), getOrderFormCatalog(pf.code), listPacks(),
  ]);
  if (!order) notFound();
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-6 md:px-8">
      ...
      <OrderForm platform={pf.code} products={products} sizes={sizes} provinces={provinces} postcodes={postcodes} initial={order} productCodes={productCodes} productTypes={productTypes} discontinued={discontinued} discInStock={discInStock} testerStock={testerStock} branches={wsCat.branches} catalogSizes={wsCat.catalogSizes} packs={packs} />
    </div>
  );
}
