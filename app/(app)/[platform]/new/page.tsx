...
import { getProducts, getSizes, getProvinces, getPostcodes, getProductCodes, getProductTypes, getBlockedSizesForOrder, getDiscInStock, getOrderFormCatalog } from "@/lib/queries";
...
export default async function NewOrderPage({ params }: { params: Promise<{ platform: string }> }) {
  ...
  const [products, sizes, provinces, postcodes, productCodes, productTypes, discontinued, discInStock, wsCat] = await Promise.all([
    getProducts(), getSizes(), getProvinces(), getPostcodes(), getProductCodes(), getProductTypes(), getBlockedSizesForOrder(), getDiscInStock(), getOrderFormCatalog(pf.code),
  ]);
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-6 md:px-8">
      ...
      <OrderForm platform={pf.code} products={products} sizes={sizes} provinces={provinces} postcodes={postcodes} productCodes={productCodes} productTypes={productTypes} discontinued={discontinued} discInStock={discInStock} branches={wsCat.branches} catalogSizes={wsCat.catalogSizes} />
    </div>
  );
}
